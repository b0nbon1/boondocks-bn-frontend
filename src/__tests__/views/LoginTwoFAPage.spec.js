...
describe("<LoginTwoFAPage />", () => {
  it("should render the page", async () => {
    ...
    const { getByTestId, findByText } = render(
      <BrowserRouter>
        <LoginTwoFAPage {...props} />
      </BrowserRouter>
    );

    const login2FAPage = await waitForElement(
      () => getByTestId("login_2fa_page")
    );

    expect(login2FAPage).toBeInTheDocument();
  });
  ...
  it("should render the page", async () => {
    ...
    const sendTextBtn = await waitForElement(
      () => getByTestId("send_text_btn")
    );
    expect(sendTextBtn).toBeInTheDocument();
    fireEvent.click(sendTextBtn);

    const smsTextInput = await waitForElement(
      () => findByLabelText(/Token/)
    );

    fireEvent.change(smsTextInput, { target: { value: "565655" } });

    const verifyBtn = await waitForElement(() => getByTestId("verify_btn"));
  ...
  it("should render the page", async () => {
    const setTimeout = jest.useFakeTimers();
